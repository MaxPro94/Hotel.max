...
if (isset($_GET['page'])) {
    if (isset($_SESSION['user_id'])) {
        ...
        $requete_reserv = $maDB->prepare("SELECT * FROM reservation WHERE id_utilisateur = :id_utilisateur ORDER BY id_reservation DESC LIMIT 1");
        $requete_reserv->execute([
            'id_utilisateur' => $_SESSION['user_id']
        ]);
        $reservation = $requete_reserv->fetch();
